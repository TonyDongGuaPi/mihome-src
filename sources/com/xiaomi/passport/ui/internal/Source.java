package com.xiaomi.passport.ui.internal;

import android.os.AsyncTask;
import com.xiaomi.passport.ui.internal.Result;
import kotlin.Metadata;
import kotlin.Unit;
import kotlin.jvm.functions.Function0;
import kotlin.jvm.functions.Function1;
import kotlin.jvm.internal.Intrinsics;
import org.jetbrains.annotations.NotNull;

@Metadata(bv = {1, 0, 2}, d1 = {"\u00000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u0000\n\u0002\b\u0002\n\u0002\u0010\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u0003\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0007\n\u0002\u0018\u0002\n\u0002\b\u0005\b&\u0018\u0000 \u0017*\u0004\b\u0000\u0010\u00012\u00020\u0002:\u0001\u0017B\u0005¢\u0006\u0002\u0010\u0003J.\u0010\u0004\u001a\u00020\u00052\u0012\u0010\u0006\u001a\u000e\u0012\u0004\u0012\u00028\u0000\u0012\u0004\u0012\u00020\u00050\u00072\u0012\u0010\b\u001a\u000e\u0012\u0004\u0012\u00020\t\u0012\u0004\u0012\u00020\u00050\u0007J\"\u0010\u0004\u001a\u00020\u00052\u0018\u0010\n\u001a\u0014\u0012\n\u0012\b\u0012\u0004\u0012\u00028\u00000\u000b\u0012\u0004\u0012\u00020\u00050\u0007H\u0002J\"\u0010\f\u001a\u00020\u00052\u0018\u0010\n\u001a\u0014\u0012\n\u0012\b\u0012\u0004\u0012\u00028\u00000\u000b\u0012\u0004\u0012\u00020\u00050\u0007H\u0002J\u001a\u0010\r\u001a\u00020\u00052\u0012\u0010\u0006\u001a\u000e\u0012\u0004\u0012\u00028\u0000\u0012\u0004\u0012\u00020\u00050\u0007J\u000b\u0010\u000e\u001a\u00028\u0000¢\u0006\u0002\u0010\u000fJ\"\u0010\u000e\u001a\u00020\u00052\u0018\u0010\n\u001a\u0014\u0012\n\u0012\b\u0012\u0004\u0012\u00028\u00000\u000b\u0012\u0004\u0012\u00020\u00050\u0007H\u0002J5\u0010\u0010\u001a\b\u0012\u0004\u0012\u0002H\u00110\u0000\"\u0004\b\u0001\u0010\u00112!\u0010\u0012\u001a\u001d\u0012\u0013\u0012\u00118\u0000¢\u0006\f\b\u0013\u0012\b\b\u0014\u0012\u0004\b\b(\u0015\u0012\u0004\u0012\u0002H\u00110\u0007J\r\u0010\u0016\u001a\u00028\u0000H&¢\u0006\u0002\u0010\u000f¨\u0006\u0018"}, d2 = {"Lcom/xiaomi/passport/ui/internal/Source;", "V", "", "()V", "get", "", "success", "Lkotlin/Function1;", "fail", "", "observer", "Lcom/xiaomi/passport/ui/internal/Result;", "getAsync", "getSuccess", "getSync", "()Ljava/lang/Object;", "next", "T", "func1", "Lkotlin/ParameterName;", "name", "value", "process", "Companion", "passportui_release"}, k = 1, mv = {1, 1, 10})
public abstract class Source<V> {
    public static final Companion Companion = new Companion((DefaultConstructorMarker) null);

    public abstract V process();

    @Metadata(bv = {1, 0, 2}, d1 = {"\u0000\u001a\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0002\b\u0002\n\u0002\u0018\u0002\n\u0002\b\u0002\n\u0002\u0018\u0002\n\u0000\b\u0003\u0018\u00002\u00020\u0001B\u0007\b\u0002¢\u0006\u0002\u0010\u0002J \u0010\u0003\u001a\b\u0012\u0004\u0012\u0002H\u00050\u0004\"\u0004\b\u0001\u0010\u00052\f\u0010\u0006\u001a\b\u0012\u0004\u0012\u0002H\u00050\u0007¨\u0006\b"}, d2 = {"Lcom/xiaomi/passport/ui/internal/Source$Companion;", "", "()V", "from", "Lcom/xiaomi/passport/ui/internal/Source;", "V", "func", "Lkotlin/Function0;", "passportui_release"}, k = 1, mv = {1, 1, 10})
    public static final class Companion {
        private Companion() {
        }

        public /* synthetic */ Companion(DefaultConstructorMarker defaultConstructorMarker) {
            this();
        }

        @NotNull
        public final <V> Source<V> from(@NotNull Function0<? extends V> function0) {
            Intrinsics.f(function0, "func");
            return new Source$Companion$from$1(function0);
        }
    }

    private final void get(Function1<? super Result<V>, Unit> function1) {
        boolean enable_test = SourceTool.Companion.getENABLE_TEST();
        if (enable_test) {
            getSync(function1);
        } else if (!enable_test) {
            getAsync(function1);
        }
    }

    public final void get(@NotNull Function1<? super V, Unit> function1, @NotNull Function1<? super Throwable, Unit> function12) {
        Intrinsics.f(function1, "success");
        Intrinsics.f(function12, "fail");
        get(new Source$get$1(function1, function12));
    }

    public final void getSuccess(@NotNull Function1<? super V, Unit> function1) {
        Intrinsics.f(function1, "success");
        get(function1, Source$getSuccess$1.INSTANCE);
    }

    @NotNull
    public final <T> Source<T> next(@NotNull Function1<? super V, ? extends T> function1) {
        Intrinsics.f(function1, "func1");
        return new Source$next$1(this, function1);
    }

    private final void getSync(Function1<? super Result<V>, Unit> function1) {
        try {
            function1.invoke(new Result.Success(process()));
        } catch (Throwable th) {
            function1.invoke(new Result.Failure(th));
        }
    }

    public final V getSync() {
        return process();
    }

    private final void getAsync(Function1<? super Result<V>, Unit> function1) {
        AsyncTask.THREAD_POOL_EXECUTOR.execute(new Source$getAsync$1(this, function1));
    }
}
