    ...
    public static XiaomiUserCoreInfo queryXiaomiUserCoreInfo(Context context, XMPassportInfo xMPassportInfo, List<XiaomiUserCoreInfo.Flag> list) {
        ...
        while (i < 2) {
            try {
                ...
                AccountLog.e(TAG, "invalid response when get user info", e);
            } catch (CipherException e2) {
                AccountLog.e(TAG, "CipherException when get user info", e2);
            } catch (IOException e3) {
                AccountLog.e(TAG, "IOException when get user info", e3);
            } catch (AuthenticationFailureException e4) {
                AccountLog.e(TAG, "auth failure when get user info", e4);
                xMPassportInfo.refreshAuthToken(context);
                i++;
            } catch (AccessDeniedException e5) {
                AccountLog.e(TAG, "access denied when get user info", e5);
            }
        }
        return null;
    }
