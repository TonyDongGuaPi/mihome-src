...
public class AccountServerTimeComputer implements ServerTimeUtil.IServerTimeComputer {
    ...
    static {
        for (String url : new String[]{URLs.ACCOUNT_DOMAIN, URLs.CA_ACCOUNT_DOMAIN}) {
            try {
                acceptHosts.add(new URL(url).getHost().toLowerCase());
    ...
    public long getSystemTimeDiff() {
        return getSP().getLong(SP_KEY_SYSTEM_TIME_DIFF, 0);
    }
