...
public class DeviceController extends SpecDevice {
    ...
    public PropertyParam newPropertyParam(int i, int i2) {
        return newPropertyParam(i, i2, (Object) null);
    }
        ...
        if (!TextUtils.isEmpty(str)) {
            try {
                ...
                for (int i = 0; i < jSONArray.length(); i++) {
                    ...
                    if (split.length >= 3) {
                        PropertyController propertyController = getPropertyController(Integer.valueOf(split[1]).intValue(), Integer.valueOf(split[2]).intValue());
                        if (!(propertyController == null || (optJSONArray = optJSONObject.optJSONArray("value")) == null)) {
