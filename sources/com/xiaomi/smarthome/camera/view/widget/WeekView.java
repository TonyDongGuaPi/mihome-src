...
public class WeekView extends LinearLayout implements View.OnClickListener {
    private static final long INTERVAL_Of_24_HOUR = 86400000;
    ...
    private void initTextSize(Context context) {
        this.mBgSqareWidth = (int) ((((float) ((WindowManager) getContext().getSystemService("window")).getDefaultDisplay().getWidth()) * 0.9f) / 7.0f);
        this.mMonthTextSize = (int) (((float) this.mBgSqareWidth) / 4.0f);
        this.mDayTextSize = (int) (((float) this.mBgSqareWidth) / 3.0f);
    }
    ...
    private View prepareDayView(int i, int i2) {
        ...
        linearLayout2.addView(linearLayout, new LinearLayout.LayoutParams(this.mBgSqareWidth, this.mBgSqareWidth));
            ...
            for (int i2 = 0; i2 < linearLayout.getChildCount(); i2++) {
                ...
                if (childAt instanceof TextView) {
                    if (((Integer) childAt.getTag()).intValue() == 0) {
                        ...
                        ((TextView) childAt).setTextColor(Color.argb(127, 0, 0, 0));
                    }
                }
            }
        }
