...
public class AlarmSelectView extends View implements View.OnTouchListener {
    ...
    public AlarmSelectView(Context context, AttributeSet attributeSet, int i) {
        ...
        this.mLineCount = 8;
        this.mRawCount = 4;
        this.mOldItems = (int[][]) Array.newInstance(int.class, new int[]{4, 8});
        this.mItems = (int[][]) Array.newInstance(int.class, new int[]{4, 8});
        this.mDownItems = (int[][]) Array.newInstance(int.class, new int[]{4, 8});
        init();
    }
    ...
    private void init() {
        ...
        this.mPainColors[1] = Color.argb(0, 122, TbsListener.ErrorCode.HOST_CONTEXT_IS_NULL, 0);
        this.mPainColors[2] = Color.argb(0, 255, 192, 0);
        this.mPainColors[3] = Color.argb(0, TbsListener.ErrorCode.DEXOAT_EXCEPTION, 0, 0);
        this.mPainLine = new Paint();
        this.mPainLine.setAntiAlias(true);
        this.mPainLine.setStrokeWidth(3.0f);
        this.mPainLine.setColor(getResources().getColor(R.color.camera_white_bg));
    ...
    public void onMeasure(int i, int i2) {
        ...
        this.mHeight = this.mWidth / this.mRatio;
        this.mSpaceWidth = this.mWidth / 8.0f;
        this.mSpaceHeight = this.mHeight / 4.0f;
        super.onMeasure(i, View.MeasureSpec.makeMeasureSpec((int) this.mHeight, mode));
    }
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        if (motionEvent.getAction() == 0) {
            ...
            int i = this.mDownItems[min2][min];
            if (i == 3) {
                this.mDownItems[min2][min] = 0;
            } else {
                this.mDownItems[min2][min] = i + 1;
            }
            ...
            if (this.mDownItems[min4][min3] == this.mItems[min4][min3]) {
                int i2 = this.mDownItems[min4][min3];
                if (i2 == 3) {
                    this.mDownItems[min4][min3] = 0;
                } else {
                    this.mDownItems[min4][min3] = i2 + 1;
                }
            }
            ...
            if (this.mDownItems[min6][min5] == this.mItems[min6][min5]) {
                int i3 = this.mItems[min6][min5];
                if (i3 == 3) {
                    this.mDownItems[min6][min5] = 0;
                } else {
                    this.mDownItems[min6][min5] = i3 + 1;
                }
                invalidate();
            }
