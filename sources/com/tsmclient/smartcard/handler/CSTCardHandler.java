...
public class CSTCardHandler extends BaseTransCardHandler {
    private static final ByteArray FID_00 = ByteArray.wrap(new byte[]{Constants.TagName.CARD_APP_ACTIVATION_STATUS, 0});
    private static final ByteArray FID_01 = ByteArray.wrap(new byte[]{Constants.TagName.CARD_APP_ACTIVATION_STATUS, 1});
    private static final ByteArray GET_BALANCE_CMD = ByteArray.wrap(new byte[]{Byte.MIN_VALUE, Constants.TagName.ORDER_TRADE_STATUSES, 0, 2, 4});
    private static final ByteArray GET_CARDNUM_CMD = ByteArray.wrap(new byte[]{0, -80, Constants.TagName.PREDEPOSIT_TYPE, 0, 30});
    private static final ByteArray GET_REAL_CARDNUM_CMD = ByteArray.wrap(new byte[]{Byte.MIN_VALUE, -54, 0, 0, 9});
    private static final ByteArray READ_CARD_STATAUS_CMD = ByteArray.wrap(new byte[]{0, Constants.TagName.APP_TYPE, 2, Constants.TagName.EUID, 48});
    private final String HAS_BEEN_BACK_CARD = "01111";
    private final String ILLEGAL_CARD_NUMBER = "0000000000000000";

    /* access modifiers changed from: protected */
    public String getCardType() {
        return CardConstants.CST;
    }
    ...
    public Map<String, String> getCardNumAndValidDate() throws IOException, UnProcessableCardException {
        ...
        if (!bytesToHexString.equals("0000000000000000")) {
            ...
            hashMap.put(CardConstants.KEY_ACCOUNT_REAL_NUM, Coder.bytesToHexString(ByteArray.wrap(transceive2, 1, 4).toBytes()));
            return hashMap;
        }
    ...
    public void readRecord(ArrayList<TradeLog> arrayList, boolean z) throws IOException {
        readRecord(arrayList, true, Constants.TagName.USER_PLATFORM_TYPE, (byte) 23, false);
    ...
    public void readCardStatus(Map<String, String> map) throws IOException, UnProcessableCardException {
        if (map != null) {
            ...
            if (transceive.length < 5) {
                map.put(CardConstants.STATUS_NEGATIVE, "1");
                return;
            }
            map.put(CardConstants.STATUS_NEGATIVE, String.valueOf(TextUtils.equals(String.format("%08d", new Object[]{Integer.valueOf(Integer.parseInt(Integer.toBinaryString(ByteArray.wrap(transceive).get(4))))}).substring(0, 5), "01111") ? 1 : 0));
        }
    }
}
