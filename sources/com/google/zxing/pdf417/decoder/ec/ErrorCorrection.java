...
public final class ErrorCorrection {
    ...
    private ModulusPoly[] runEuclideanAlgorithm(ModulusPoly modulusPoly, ModulusPoly modulusPoly2, int i) throws ChecksumException {
        ...
        while (modulusPoly6.getDegree() >= i / 2) {
            if (!modulusPoly6.isZero()) {
                ModulusPoly zero2 = this.field.getZero();
                int inverse = this.field.inverse(modulusPoly6.getCoefficient(modulusPoly6.getDegree()));
                while (modulusPoly5.getDegree() >= modulusPoly6.getDegree() && !modulusPoly5.isZero()) {
                    int degree = modulusPoly5.getDegree() - modulusPoly6.getDegree();
                    ...
                    modulusPoly5 = modulusPoly5.subtract(modulusPoly6.multiplyByMonomial(degree, multiply));
                }
                ModulusPoly modulusPoly7 = modulusPoly5;
                modulusPoly5 = modulusPoly6;
                modulusPoly6 = modulusPoly7;
                ModulusPoly modulusPoly8 = one;
        ...
        if (coefficient != 0) {
            ...
            return new ModulusPoly[]{one.multiply(inverse2), modulusPoly6.multiply(inverse2)};
        }
        throw ChecksumException.getChecksumInstance();
    }
