...
public class ISO8601Utils {
    ...
    public static String format(Date date, boolean z, TimeZone timeZone) {
        ...
        StringBuilder sb = new StringBuilder("yyyy-MM-ddThh:mm:ss".length() + (z ? ".sss".length() : 0) + (timeZone.getRawOffset() == 0 ? Field.f3009a : "+hh:mm").length());
        padInt(sb, gregorianCalendar.get(1), "yyyy".length());
        char c = '-';
        sb.append('-');
        padInt(sb, gregorianCalendar.get(2) + 1, "MM".length());
        sb.append('-');
        padInt(sb, gregorianCalendar.get(5), d.s.length());
        sb.append(PatternFormatter.THROWABLE_CONVERSION_CHAR);
