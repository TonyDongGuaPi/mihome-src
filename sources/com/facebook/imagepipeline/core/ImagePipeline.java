...
public class ImagePipeline {
    ...
    public DataSource<Boolean> isInDiskCache(ImageRequest imageRequest) {
        ...
        this.mMainBufferedDiskCache.contains(encodedCacheKey).b(new Continuation<Boolean, Task<Boolean>>() {
            public Task<Boolean> then(Task<Boolean> task) throws Exception {
                if (task.d() || task.e() || !task.f().booleanValue()) {
                    return ImagePipeline.this.mSmallImageBufferedDiskCache.contains(encodedCacheKey);
                }
                return Task.a(true);
            }
        }).a(new Continuation<Boolean, Void>() {
            public Void then(Task<Boolean> task) throws Exception {
                create.setResult(Boolean.valueOf(!task.d() && !task.e() && task.f().booleanValue()));
                return null;
            }
        });
        return create;
