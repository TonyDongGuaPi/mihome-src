        ...
        return new Continuation<EncodedImage, Void>() {
            public Void then(Task<EncodedImage> task) throws Exception {
                if (PartialDiskCacheProducer.isTaskCancelled(task)) {
                    ...
                    listener.onProducerFinishWithFailure(id, PartialDiskCacheProducer.PRODUCER_NAME, task.g(), (Map<String, String>) null);
                    ...
                    if (f != null) {
                        listener.onProducerFinishWithSuccess(id, PartialDiskCacheProducer.PRODUCER_NAME, PartialDiskCacheProducer.getExtraMap(listener, id, true, f.getSize()));
                        BytesRange max = BytesRange.toMax(f.getSize() - 1);
                        f.setBytesRange(max);
                        int size = f.getSize();
                        ...
                        if (max.contains(imageRequest.getBytesRange())) {
                            ...
                            consumer2.onNewResult(f, 9);
                        } else {
                            consumer2.onNewResult(f, 8);
                            PartialDiskCacheProducer.this.startInputProducer(consumer2, new SettableProducerContext(ImageRequestBuilder.fromRequest(imageRequest).setBytesRange(BytesRange.from(size - 1)).build(), producerContext2), cacheKey2, f);
                        }
                    } else {
                        ...
                        PartialDiskCacheProducer.this.startInputProducer(consumer2, producerContext2, cacheKey2, f);
                    }
                }
                return null;
            }
