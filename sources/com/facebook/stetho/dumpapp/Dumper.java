...
public class Dumper {
    ...
    private int doDump(InputStream inputStream, PrintStream printStream, PrintStream printStream2, String[] strArr) throws ParseException, DumpException {
        CommandLine b = this.mParser.b(this.mGlobalOptions.options, strArr, true);
        if (b.hasOption(this.mGlobalOptions.optionHelp.getOpt())) {
            dumpUsage(printStream);
            return 0;
        } else if (b.hasOption(this.mGlobalOptions.optionListPlugins.getOpt())) {
            dumpAvailablePlugins(printStream);
            return 0;
        } else if (!b.getArgList().isEmpty()) {
            dumpPluginOutput(inputStream, printStream, printStream2, b);
            return 0;
        } else {
            dumpUsage(printStream2);
            return 1;
    ...
    private void dumpUsage(PrintStream printStream) {
        ...
        try {
            helpFormatter.a(printWriter, helpFormatter.a(), this.mGlobalOptions.options, helpFormatter.b(), helpFormatter.c());
        } finally {
            printWriter.flush();
        }
    }
