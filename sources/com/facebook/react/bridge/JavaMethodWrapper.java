...
public class JavaMethodWrapper implements NativeModule.NativeMethod {
    ...
    private static char paramTypeToChar(Class cls) {
        ...
        if (cls == Promise.class) {
            return PatternFormatter.PRIORITY_CONVERSION_CHAR;
        }
    ...
    private static char commonTypeToChar(Class cls) {
        ...
        if (cls == Integer.TYPE) {
            return PatternFormatter.CLIENT_ID_CONVERSION_CHAR;
        }
        if (cls == Integer.class) {
            return 'I';
        }
        if (cls == Double.TYPE) {
            return PatternFormatter.DATE_CONVERSION_CHAR;
        }
        if (cls == Double.class) {
            return 'D';
        }
    ...
    public void invoke(JSInstance jSInstance, ReadableArray readableArray) {
        ...
        try {
            ...
            throw new NativeArgumentsParseException(e.getMessage() + " (constructing arguments for " + str + " at argument index " + getAffectedRange(i, this.mArgumentExtractors[i2].getJSArgumentsNeeded()) + Operators.BRACKET_END_STR, e);
