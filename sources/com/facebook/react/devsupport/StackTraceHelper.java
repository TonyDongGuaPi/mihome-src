    ...
    public static StackFrame[] convertJsStackTrace(@Nullable ReadableArray readableArray) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (type == ReadableType.Map) {
                ...
                stackFrameArr[i] = new StackFrameImpl(map.getString("file"), map.getString("methodName"), (!map.hasKey(LINE_NUMBER_KEY) || map.isNull(LINE_NUMBER_KEY)) ? -1 : map.getInt(LINE_NUMBER_KEY), (!map.hasKey("column") || map.isNull("column")) ? -1 : map.getInt("column"));
    ...
    public static StackFrame[] convertJsStackTrace(JSONArray jSONArray) {
        ...
        while (i < length) {
            try {
                ...
                stackFrameArr[i] = new StackFrameImpl(jSONObject.getString("file"), jSONObject.getString("methodName"), (!jSONObject.has(LINE_NUMBER_KEY) || jSONObject.isNull(LINE_NUMBER_KEY)) ? -1 : jSONObject.getInt(LINE_NUMBER_KEY), (!jSONObject.has("column") || jSONObject.isNull("column")) ? -1 : jSONObject.getInt("column"));
                i++;
