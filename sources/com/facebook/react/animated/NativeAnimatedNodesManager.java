...
class NativeAnimatedNodesManager implements EventDispatcherListener {
    ...
    public void createAnimatedNode(int i, ReadableMap readableMap) {
        ...
        if (this.mAnimatedNodes.get(i) == null) {
            ...
            } else if (Constants.XMLNode.KEY_MODULUS.equals(string)) {
    ...
    private void updateNodes(List<AnimatedNode> list) {
        ...
        for (AnimatedNode next : list) {
            if (next.mBFSColor != this.mAnimatedGraphBFSColor) {
                next.mBFSColor = this.mAnimatedGraphBFSColor;
                i++;
                arrayDeque.add(next);
            }
        }
        while (!arrayDeque.isEmpty()) {
            ...
            if (animatedNode.mChildren != null) {
                int i2 = i;
                for (int i3 = 0; i3 < animatedNode.mChildren.size(); i3++) {
                    AnimatedNode animatedNode2 = animatedNode.mChildren.get(i3);
        ...
        for (AnimatedNode next2 : list) {
            if (next2.mActiveIncomingNodes == 0 && next2.mBFSColor != this.mAnimatedGraphBFSColor) {
                next2.mBFSColor = this.mAnimatedGraphBFSColor;
                i4++;
        ...
        while (!arrayDeque.isEmpty()) {
            ...
            if (animatedNode3.mChildren != null) {
                int i5 = i4;
                for (int i6 = 0; i6 < animatedNode3.mChildren.size(); i6++) {
                    AnimatedNode animatedNode4 = animatedNode3.mChildren.get(i6);
