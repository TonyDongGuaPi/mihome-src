...
public class LoadPaymentsMapping extends AsyncTask {
    ...
    public static String loadMappingSync(String str, int i, Context context2) {
        return "0".equals(b.b(context2).f().getMode()) ? getBundledMapping(context2) : Network.getResponseFromQuery(context2, ((Constants.getServerUrl(i + "") + "bankMapping.do" + Constants.CODE_PAYMENT_MAPPING_VERSION + "&clientId=3") + "&sellerId=" + str) + "&action=sdkMapping");
    }
    ...
    public void onPostExecute(String str) {
        if ("CACHED".equals(str)) {
            ...
            callback((PaymentsMappingAPIResponse) null);
        } else {
            try {
