...
final class MarkableInputStream extends InputStream {
    ...
    private final InputStream f8874in;
    private long limit;
    private long offset;
    private long reset;
    ...
    public MarkableInputStream(InputStream inputStream, int i) {
        this.defaultMark = -1;
        this.f8874in = !inputStream.markSupported() ? new BufferedInputStream(inputStream, i) : inputStream;
    }
    ...
    private void setLimit(long j) {
        try {
            if (this.reset >= this.offset || this.offset > this.limit) {
                ...
                this.f8874in.mark((int) (j - this.offset));
            } else {
                this.f8874in.reset();
                this.f8874in.mark((int) (j - this.reset));
    ...
    private void skip(long j, long j2) throws IOException {
        while (j < j2) {
            long skip = this.f8874in.skip(j2 - j);
            if (skip == 0) {
                if (read() != -1) {
                    skip = 1;
                } else {
    ...
    public int read(byte[] bArr) throws IOException {
        int read = this.f8874in.read(bArr);
        if (read != -1) {
    ...
    public int read(byte[] bArr, int i, int i2) throws IOException {
        int read = this.f8874in.read(bArr, i, i2);
        if (read != -1) {
            this.offset += (long) read;
        }
        return read;
    ...
    public long skip(long j) throws IOException {
        long skip = this.f8874in.skip(j);
        this.offset += skip;
        return skip;
    }

    public int available() throws IOException {
        return this.f8874in.available();
    }
    ...
    public boolean markSupported() {
        return this.f8874in.markSupported();
    }
}
