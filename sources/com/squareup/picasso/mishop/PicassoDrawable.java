...
final class PicassoDrawable extends BitmapDrawable {
    ...
    private final float fadeValue;
    private final Picasso.LoadedFrom loadedFrom;
    Drawable placeholder;
    long startTimeMillis;

    static void setBitmap(ImageView imageView, Context context, Bitmap bitmap, Picasso.LoadedFrom loadedFrom2, boolean z, float f, boolean z2) {
        Drawable drawable = imageView.getDrawable();
        if (drawable instanceof AnimationDrawable) {
            ((AnimationDrawable) drawable).stop();
        }
        imageView.setImageDrawable(new PicassoDrawable(context, bitmap, drawable, loadedFrom2, z, f, z2));
    }
    ...
    PicassoDrawable(Context context, Bitmap bitmap, Drawable drawable, Picasso.LoadedFrom loadedFrom2, boolean z, float f, boolean z2) {
        super(context.getResources(), bitmap);
        this.fadeValue = f <= 0.0f ? 200.0f : f;
        this.debugging = z2;
    ...
    public void draw(Canvas canvas) {
        if (!this.animating) {
            ...
            float uptimeMillis = ((float) (SystemClock.uptimeMillis() - this.startTimeMillis)) / this.fadeValue;
            if (uptimeMillis >= 1.0f) {
                this.animating = false;
                this.placeholder = null;
                super.draw(canvas);
            } else {
                if (this.placeholder != null && (!(this.placeholder instanceof PicassoDrawable) || !((PicassoDrawable) this.placeholder).animating)) {
