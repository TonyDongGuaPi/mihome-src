...
class RCTCameraViewFinder extends TextureView implements Camera.PreviewCallback, TextureView.SurfaceTextureListener {
    ...
    private void initBarcodeReader(List<String> list) {
        ...
        this._multiFormatReader.a((Map<DecodeHintType, ?>) enumMap);
    }
    ...
    private class ReaderAsyncTask extends AsyncTask<Void, Void, Result> {
        ...
        private Result getBarcode(int i, int i2, boolean z) {
            ...
            try {
                ...
                if (z) {
                    binaryBitmap = new BinaryBitmap(new HybridBinarizer(planarYUVLuminanceSource.d()));
                } else {
                    binaryBitmap = new BinaryBitmap(new HybridBinarizer(planarYUVLuminanceSource));
                }
                Result b = RCTCameraViewFinder.this._multiFormatReader.b(binaryBitmap);
                RCTCameraViewFinder.this._multiFormatReader.a();
                return b;
            } catch (Throwable th) {
                RCTCameraViewFinder.this._multiFormatReader.a();
                throw th;
        ...
        private void rotateImage(int i, int i2) {
            ...
            for (int i3 = 0; i3 < i; i3++) {
                for (int i4 = 0; i4 < i2; i4++) {
                    int i5 = (i3 * i2) + i4;
                    int i6 = (((i4 * i) + i) - i3) - 1;
                    if (i5 >= 0 && i5 < this.imageData.length && i6 >= 0 && i6 < this.imageData.length) {
                        bArr[i6] = this.imageData[i5];
                    }
                }
            }
            this.imageData = bArr;
        ...
        public void onPostExecute(Result result) {
            RCTCameraViewFinder.this.sendEvent(result);
        }
    }

    /* access modifiers changed from: package-private */
    public void sendEvent(Result result) {
        if (getContext() instanceof ThemedReactContext) {
            ...
            if (c != null) {
                for (ResultPoint resultPoint : c) {
                    ...
                    createMap2.putString(Constants.Name.Y, String.valueOf(resultPoint.b()));
                    createArray.pushMap(createMap2);
                }
            }
            createMap.putArray("bounds", createArray);
            createMap.putString("data", result.a());
            createMap.putString("type", result.d().toString());
