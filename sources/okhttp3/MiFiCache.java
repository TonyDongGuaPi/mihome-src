...
public final class MiFiCache implements Closeable, Flushable {
    private static final int ENTRY_BODY = 1;
    private static final int ENTRY_COUNT = 2;
    private static final int ENTRY_METADATA = 0;
    private static final int VERSION = 201105;
    /* access modifiers changed from: private */
    public static GetCacheUrlCallback sGetCacheUrlCallback = $$Lambda$MiFiCache$KH5ht7f6ItycbqORAAz7smhAhk.INSTANCE;
    ...
    public interface GetCacheUrlCallback {
        HttpUrl cacheUrl(HttpUrl httpUrl);
    }

    static /* synthetic */ HttpUrl lambda$static$0(HttpUrl httpUrl) {
        return httpUrl;
    }

    public MiFiCache(File file, long j) {
        this(file, j, FileSystem.SYSTEM);
    }

    MiFiCache(File file, long j, FileSystem fileSystem) {
        this.internalCache = new InternalCache() {
            public Response get(Request request) throws IOException {
                return MiFiCache.this.get(request);
            }

            public CacheRequest put(Response response) throws IOException {
                return MiFiCache.this.put(response);
            }

            public void remove(Request request) throws IOException {
                MiFiCache.this.remove(request);
            }

            public void update(Response response, Response response2) {
                MiFiCache.this.update(response, response2);
            }

            public void trackConditionalCacheHit() {
                MiFiCache.this.trackConditionalCacheHit();
            }

            public void trackResponse(CacheStrategy cacheStrategy) {
                MiFiCache.this.trackResponse(cacheStrategy);
            }
        };
        this.cache = DiskLruCache.create(fileSystem, file, VERSION, 2, j);
    }

    public static void setGetCacheUrlCallback(@Nonnull GetCacheUrlCallback getCacheUrlCallback) {
        sGetCacheUrlCallback = getCacheUrlCallback;
    }

    public static String key(HttpUrl httpUrl) {
        if (sGetCacheUrlCallback != null) {
            httpUrl = sGetCacheUrlCallback.cacheUrl(httpUrl);
        }
    ...
    public Iterator<String> urls() throws IOException {
        return new Iterator<String>() {
            boolean canRemove;
            final Iterator<DiskLruCache.Snapshot> delegate = MiFiCache.this.cache.snapshots();
            @Nullable
            String nextUrl;
    ...
    private final class CacheRequestImpl implements CacheRequest {
        ...
        CacheRequestImpl(final DiskLruCache.Editor editor2) {
            ...
            this.body = new ForwardingSink(this.cacheOut, MiFiCache.this) {
                public void close() throws IOException {
                    synchronized (MiFiCache.this) {
                        if (!CacheRequestImpl.this.done) {
                            CacheRequestImpl.this.done = true;
                            MiFiCache.this.writeSuccessCount++;
                            super.close();
                            editor2.commit();
                        }
                    }
        ...
        public void abort() {
            synchronized (MiFiCache.this) {
                if (!this.done) {
                    this.done = true;
                    MiFiCache.this.writeAbortCount++;
    ...
    private static final class Entry {
        ...
        Entry(Source source) throws IOException {
            try {
                ...
                int readInt = MiFiCache.readInt(buffer);
                ...
                int readInt2 = MiFiCache.readInt(buffer);
        ...
        private List<Certificate> readCertificateList(BufferedSource bufferedSource) throws IOException {
            int readInt = MiFiCache.readInt(bufferedSource);
            if (readInt == -1) {
        ...
        public boolean matches(Request request, Response response) {
            if (MiFiCache.sGetCacheUrlCallback != null) {
                if (!TextUtils.equals(MiFiCache.sGetCacheUrlCallback.cacheUrl(HttpUrl.parse(this.url)).toString(), MiFiCache.sGetCacheUrlCallback.cacheUrl(request.url()).toString()) || !this.requestMethod.equals(request.method()) || !HttpHeaders.varyMatches(response, this.varyHeaders, request)) {
                    return false;
                }
                return true;
            } else if (!this.url.equals(request.url().toString()) || !this.requestMethod.equals(request.method()) || !HttpHeaders.varyMatches(response, this.varyHeaders, request)) {
                return false;
            } else {
                return true;
            }
