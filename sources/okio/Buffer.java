...
public final class Buffer implements Cloneable, ByteChannel, BufferedSink, BufferedSource {
    private static final byte[] DIGITS = {48, 49, 50, 51, 52, TarConstants.R, 54, 55, ScriptToolsConst.TagName.TagSerial, ScriptToolsConst.TagName.TagApdu, 97, Constants.TagName.OPERATE_TIMING, Constants.TagName.PAY_ORDER, Constants.TagName.PAY_ORDER_LIST, Constants.TagName.ORDER_TYPE, 102};
    ...
    public long readLong() {
        if (this.size >= 8) {
            ...
            int i2 = segment.limit;
            if (i2 - i < 8) {
                return ((((long) readInt()) & MessageHead.SERIAL_MAK) << 32) | (MessageHead.SERIAL_MAK & ((long) readInt()));
            }
            byte[] bArr = segment.data;
            int i3 = i + 1;
            int i4 = i3 + 1;
    ...
    public String readUtf8LineStrict(long j) throws EOFException {
        if (j >= 0) {
            ...
            throw new EOFException("\\n not found: limit=" + Math.min(size(), j) + " content=" + buffer.readByteString().hex() + Typography.E);
        }
        ...
        if (this.size != 0) {
            ...
            if (this.size >= j) {
                while (i2 < i) {
                    long j2 = (long) i2;
                    byte b4 = getByte(j2);
                    if ((b4 & Constants.TagName.STATION_ENAME) == 128) {
                        b2 = (b2 << 6) | (b4 & Constants.TagName.CARD_APP_ACTIVATION_STATUS);
                        i2++;
                    } else {
                        skip(j2);
                        return REPLACEMENT_CHARACTER;
            ...
            throw new EOFException("size < " + i + ": " + this.size + " (to read code point prefixed 0x" + Integer.toHexString(b3) + Operators.BRACKET_END_STR);
        }
        throw new EOFException();
    }
        ...
        if (str == null) {
            ...
            while (i < i2) {
                ...
                if (charAt < 128) {
                    ...
                    writeByte((int) (charAt & Operators.CONDITION_IF) | 128);
                    i++;
