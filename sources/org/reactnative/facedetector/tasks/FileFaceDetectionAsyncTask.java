...
public class FileFaceDetectionAsyncTask extends AsyncTask<Void, Void, SparseArray<Face>> {
    ...
    private static final String f4166a = "E_FACE_DETECTION_FAILED";
    ...
    public void onPreExecute() {
        if (this.e == null) {
            this.g.reject(f4166a, "You have to provide an URI of an image.");
            cancel(true);
            return;
        }
        ...
        if (this.f == null) {
            ...
            promise.reject(f4166a, "Invalid URI provided: `" + this.e + "`.");
            cancel(true);
            return;
        }
        if (!(this.f.startsWith(this.j.getCacheDir().getPath()) || this.f.startsWith(this.j.getFilesDir().getPath()))) {
            this.g.reject(f4166a, "The image has to be in the local app's directories.");
            cancel(true);
        } else if (!new File(this.f).exists()) {
            Promise promise2 = this.g;
            promise2.reject(f4166a, "The file does not exist. Given path: `" + this.f + "`.");
            cancel(true);
        }
    }
    ...
    public SparseArray<Face> doInBackground(Void... voidArr) {
        ...
        try {
            this.l = new ExifInterface(this.f).getAttributeInt(ExifInterface.TAG_ORIENTATION, 0);
        } catch (IOException e2) {
            Log.e(f4166a, "Reading orientation from file `" + this.f + "` failed.", e2);
        }
