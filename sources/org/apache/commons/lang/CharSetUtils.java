package org.apache.commons.lang;

import org.apache.commons.lang.text.StrBuilder;

public class CharSetUtils {
    public static CharSet a(String[] strArr) {
        if (strArr == null) {
            return null;
        }
        return new CharSet(strArr);
    }

    public static String a(String str, String str2) {
        if (StringUtils.a(str) || StringUtils.a(str2)) {
            return str;
        }
        return a(str, new String[]{str2});
    }

    public static String a(String str, String[] strArr) {
        if (StringUtils.a(str) || ArrayUtils.e((Object[]) strArr)) {
            return str;
        }
        CharSet instance = CharSet.getInstance(strArr);
        StrBuilder strBuilder = new StrBuilder(str.length());
        char[] charArray = str.toCharArray();
        int length = charArray.length;
        char c = ' ';
        for (int i = 0; i < length; i++) {
            char c2 = charArray[i];
            if (!instance.contains(c2) || c2 != c || i == 0) {
                strBuilder.a(c2);
                c = c2;
            }
        }
        return strBuilder.toString();
    }

    public static int b(String str, String str2) {
        if (StringUtils.a(str) || StringUtils.a(str2)) {
            return 0;
        }
        return b(str, new String[]{str2});
    }

    public static int b(String str, String[] strArr) {
        if (StringUtils.a(str) || ArrayUtils.e((Object[]) strArr)) {
            return 0;
        }
        CharSet instance = CharSet.getInstance(strArr);
        int i = 0;
        for (char contains : str.toCharArray()) {
            if (instance.contains(contains)) {
                i++;
            }
        }
        return i;
    }

    public static String c(String str, String str2) {
        if (str == null) {
            return null;
        }
        if (str.length() == 0 || StringUtils.a(str2)) {
            return "";
        }
        return c(str, new String[]{str2});
    }

    public static String c(String str, String[] strArr) {
        if (str == null) {
            return null;
        }
        return (str.length() == 0 || ArrayUtils.e((Object[]) strArr)) ? "" : a(str, strArr, true);
    }

    public static String d(String str, String str2) {
        if (StringUtils.a(str) || StringUtils.a(str2)) {
            return str;
        }
        return d(str, new String[]{str2});
    }

    public static String d(String str, String[] strArr) {
        return (StringUtils.a(str) || ArrayUtils.e((Object[]) strArr)) ? str : a(str, strArr, false);
    }

    private static String a(String str, String[] strArr, boolean z) {
        CharSet instance = CharSet.getInstance(strArr);
        StrBuilder strBuilder = new StrBuilder(str.length());
        char[] charArray = str.toCharArray();
        int length = charArray.length;
        for (int i = 0; i < length; i++) {
            if (instance.contains(charArray[i]) == z) {
                strBuilder.a(charArray[i]);
            }
        }
        return strBuilder.toString();
    }

    public static String a(String str, String str2, String str3) {
        if (StringUtils.a(str)) {
            return str;
        }
        StrBuilder strBuilder = new StrBuilder(str.length());
        char[] charArray = str.toCharArray();
        char[] charArray2 = str3.toCharArray();
        int length = charArray.length;
        int length2 = str3.length() - 1;
        for (int i = 0; i < length; i++) {
            int indexOf = str2.indexOf(charArray[i]);
            if (indexOf != -1) {
                if (indexOf > length2) {
                    indexOf = length2;
                }
                strBuilder.a(charArray2[indexOf]);
            } else {
                strBuilder.a(charArray[i]);
            }
        }
        return strBuilder.toString();
    }
}
